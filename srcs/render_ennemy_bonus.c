...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   render_ennemy_bonus.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: seciurte <seciurte@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/21 18:26:58 by seciurte          #+#    #+#             */
/*   Updated: 2021/11/10 15:57:40 by seciurte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_skulls(t_win *win)
{
	...
	while (i < win->nb_skulls)
	{
		...
		draw_sprite(win, win->skulls[i].x * SPT_SIZE,
			win->skulls[i].y * SPT_SIZE, win->skulls[i].index);
		...
		if (win->skulls[i].coins % SKULLS_SPEED == 0 && \
		is_in_aggro_range(i, win))
			move_skull(i, win);
		i++;
	}
}
